...
public class WordFactoryZUp : MonoBehaviour
{
	public static GameObject spawnSplittableWordBoard(string para_word,
													  ...
													  Transform para_sliceableObjPrefab,
													  ...
													  Transform para_framePrefab)
	{
		...
		// **** Spawn letter planes ****
		Vector3 nxtLetterPlaneSpawnPt = wordPlaneLeftEdge + new Vector3(widthOfLetterCell/2f,0f,-(para_sliceableObjPrefab.localScale.z/2f));
		Vector3 creepVect = new Vector3(widthOfLetterCell + widthOfSpacer,0f,0f);
		float[] rotVals = new float[3] {0,0,0};
		if(para_wordUpAxis[1])
		{
			nxtLetterPlaneSpawnPt = wordPlaneLeftEdge + new Vector3(widthOfLetterCell/2f,0f,-(para_sliceableObjPrefab.localScale.z/2f));
			rotVals = new float[3] {0,0,0};
		}
		else if(para_wordUpAxis[2])
		{
			nxtLetterPlaneSpawnPt = wordPlaneLeftEdge + new Vector3(widthOfLetterCell/2f,(para_sliceableObjPrefab.localScale.y/2f),0f);
		...
		Vector3 nxtSplitDetSpawnPt = wordPlaneLeftEdge + new Vector3(widthOfLetterCell+(widthOfSpacer/2f),0,-(para_sliceableObjPrefab.localScale.z/2f));
		if(para_wordUpAxis[1])
		{
			nxtSplitDetSpawnPt = wordPlaneLeftEdge + new Vector3(widthOfLetterCell,0,-(para_sliceableObjPrefab.localScale.z/2f));
		}
		else if(para_wordUpAxis[2])
		{
			nxtSplitDetSpawnPt = wordPlaneLeftEdge + new Vector3(widthOfLetterCell,(para_sliceableObjPrefab.localScale.y/2f),0f);
		}
		...
		for(int i=0; i<(wordStr.Length-1); i++)
		{
			...
			tmpScale.z = widthOfSplitDetect * 2f;
		...
		for(int i=0; i<wordStr.Length; i++)
		{
			GameObject nwDesignGObj = ((Transform) Instantiate(para_sliceableObjPrefab,nxtBlockSpawnPt,Quaternion.identity)).gameObject;
		...
		for(int i=0; i<(wordStr.Length-1); i++)
		{
			GameObject nwSpacerGObj = ((Transform) Instantiate(para_sliceableObjPrefab,nxtSpacerSpawnPt,Quaternion.identity)).gameObject;
		...
		// **** Create frame ****
		WordFactoryZUp.createFrame(masterWordGObj,wordStr.Length,widthOfLetterCell,widthOfWordPlane,wordPlaneLeftEdge,creepVect,para_framePrefab);
		// ********************************
	...
	public static List<GameObject> splitEntireWord(string para_wordGObjName, int[] para_splitPositions, Transform para_framePrefab)
	{
		...
		{
			nwSplits = WordFactoryZUp.performSplitOnWordBoard(ref currGObjToSplit,currSplitPos,para_framePrefab);
	...
	public static List<GameObject> performSplitOnWordBoard(ref GameObject para_hitSplitDetector, Transform para_framePrefab)
	{
		...
		return WordFactoryZUp.performSplitOnWordBoard(ref currMasterMaster,splitIndex,para_framePrefab);
	}
	
	public static List<GameObject> performSplitOnWordBoard(ref GameObject para_wordBoardToSplit, int para_splitPos, Transform para_framePrefab)
	{	
		...
		GameObject half1GObj = createSplitHalf(currMasterMaster,currMasterOverlay,half1_wordStart,half1_wordEnd,para_framePrefab);
		GameObject half2GObj = createSplitHalf(currMasterMaster,currMasterOverlay,half2_wordStart,half2_wordEnd,para_framePrefab);
											 ...
											 Transform para_framePrefab)
	{
		...
		half_masterWordGObj.name = "MasterWordGObj:"+half_wordStart+"-"+half_wordEnd;
		...
		Vector3 creepVect = new Vector3(widthOfLetterCell,0,0);
		WordFactoryZUp.createFrame(half_masterWordGObj,half_wordEnd-half_wordStart+1,widthOfLetterCell,totSizeOfHalf,leftMostPos,creepVect,para_framePrefab);
		// ********************************
								 ...
								 Transform para_framePrefab)
	{
		...
		// Top border:
		Vector3 frameTopLeft = wordPlaneLeftEdge + new Vector3(0,+ ((widthOfLetterCell/2f) + (widthOfLetterCell * 0.01f)),0) + new Vector3(0,0,(widthOfLetterCell/2f));
		
		List<Rect> frameItems = new List<Rect>();
		frameItems.Add(new Rect(frameTopLeft.x,frameTopLeft.z,widthOfWordPlane,frameBorderThickness));
		frameItems.Add(new Rect(frameTopLeft.x,frameTopLeft.z,frameBorderThickness,widthOfLetterCell));
		frameItems.Add(new Rect(frameTopLeft.x + widthOfWordPlane - frameBorderThickness,frameTopLeft.z,frameBorderThickness,widthOfLetterCell));
		frameItems.Add(new Rect(frameTopLeft.x,frameTopLeft.z - widthOfLetterCell + frameBorderThickness,widthOfWordPlane,frameBorderThickness));
		
		Vector3 nxtFrameSpacerSpawnPt = frameTopLeft + new Vector3(widthOfLetterCell,0,0);		
		for(int i=0; i<(wordLength-1); i++)
		{
			frameItems.Add(new Rect(nxtFrameSpacerSpawnPt.x,nxtFrameSpacerSpawnPt.z,frameBorderThickness,widthOfLetterCell));	
			nxtFrameSpacerSpawnPt += creepVect;
		}
		
		for(int i=0; i<frameItems.Count; i++)
		{
			GameObject nwFramePart = WorldSpawnHelper.initObjWithinWorldBounds(para_framePrefab,10,10,"FramePart",frameItems[i],null,frameTopLeft.y,new bool[3]{false,false,true});
